...
function rewrite_line() {
    ...
    #echo sed -n "/$line_regexp/p" "$filepath"
    if [[ $(sed -n "/$line_regexp/p" "$filepath") != "" ]] ; then
        new_line="$(echo "$new_line" | sed -e 's#\\#\\\\   #g')"
        new_line="$(echo "$new_line" | sed -e 's#/#\\/#g')"
        new_line="$(echo "$new_line" | sed -e 's#&#\\\&#g')"
        new_line="$(echo "$new_line" | sed -e 's#\\   #\\#g')"
        sed -i -e "s/$line_regexp/$new_line/" "$filepath"
    else
...
uninstall(){
    ...
    if [[ "$run" != "" ]]; then
        echo "$run"
        echo "fast_cd_menu is successfully uninstalled, and will take effect after next login to bash!"
    else
